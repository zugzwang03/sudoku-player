...
let level_index = 0;
let level = CONSTANTS.EMPTY[level_index];
let timer = null;
let pause = false;
let seconds = 0;
let su = undefined;
...
const loadSudoku = () => {
  ...
  game_level.innerHTML = CONSTANTS.DIFFICULTY[game.level];
...
const startGame = () => {
  ...
  game_level.innerHTML = CONSTANTS.DIFFICULTY[level_index];
  showTime(seconds);
...
document.querySelector("#btn-level").addEventListener("click", (e) => {
  level_index =
    level_index + 1 > CONSTANTS.EMPTY.length - 1 ? 0 : level_index + 1;
  level = CONSTANTS.EMPTY[level_index];
  e.target.innerHTML = CONSTANTS.DIFFICULTY[level_index];
});
